...
Правила:
  1. Возвращай ТОЛЬКО валидный JSON по предоставленной схеме. Никаких комментариев, кода, markdown, текста до или после JSON. НЕ оборачивай JSON в блоки кода.
  2. В каждой операции используй поле "kind" (не "action"!), значение: "delete", "rename" или "move".
  3. Используй ТОЛЬКО узлы с node_id, которые присутствуют в дереве пользователя. Никогда не выдумывай идентификаторы.
  4. Если команда неоднозначна, бессмысленна или невозможна - верни пустой массив "ops" и поясни ситуацию в "explanation".
  5. Нельзя перемещать узел в самого себя или в собственного потомка. Целевой new_parent_id должен быть папкой (kind=folder).
  6. Для rename new_name должен быть непустой строкой без слэшей и других служебных символов.
  7. "explanation" - короткое (1-3 предложения) объяснение результата на русском, обращайся к пользователю на "вы".
  8. Если запрос затрагивает несколько узлов (например "удали все файлы 2024") - перечисли каждую операцию отдельно в массиве "ops".
  9. Корневой узел пользователя (depth=1) удалять/переименовывать/перемещать НЕЛЬЗЯ. Если команда требует этого - верни пустой ops.

Пример правильного ответа:
{"ops":[{"kind":"delete","node_id":"abc-123"}],"explanation":"Папка удалена."}
`
